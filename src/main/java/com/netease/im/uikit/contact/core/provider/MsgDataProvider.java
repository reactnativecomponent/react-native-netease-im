...
import com.netease.im.uikit.contact.core.item.MsgItem;
...
public final class MsgDataProvider {
    ...
    private static List<MsgIndexRecord> searchSession(String query, SessionTypeEnum sessionType, String sessionId, MsgIndexRecord anchor) {
        ...
        if (anchor != null) {
            result = null;// NIMClient.getService(LuceneService.class).searchSessionNextPageBlock(query, sessionType, sessionId, anchor, 50);
        } else {
            result = null;//NIMClient.getService(LuceneService.class).searchSessionBlock(query, sessionType, sessionId);
        }
    ...
    private static List<MsgIndexRecord> searchAllSession(String query) {
        long startTime = System.currentTimeMillis();
        List<MsgIndexRecord> result = null;//NIMClient.getService(LuceneService.class).searchAllSessionBlock(query, -1);
